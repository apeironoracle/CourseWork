...
us_cities <- worldcities %>%
  ...
  rename(city = name, state = subcountry)
...
trans2usstate <- function(x){
  ...
  trans <- ifelse(last(x.split)=="usa",
         nth(x.split, -2L), trans) # some problem with washington, dc
  return(trans)
}
...
raw.data.state %>% select(user_location) %>% unique() %>% tibble

write.csv(raw.data.state, "./Process/data_usa_states_v1.csv")

# some cities with the same name convert to america
